  ...
  try {
    ...
    for (const account of accounts) {
      ...
      try {
        ...
        continue; // falha numa conta não interrompe as restantes
      }

      if (remoteTxs.length === 0) continue;

      // Categorização via ML em lote
      ...
      // Buscar categorias por nome para mapear previsão ML
