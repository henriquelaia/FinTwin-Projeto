      ...
      >
        ...
        {done ? (
          <div className="text-center py-4">
            ...
            <h2 className="text-lg font-bold text-[#101c29] mb-2">Password redefinida!</h2>
            <p className="text-sm text-[#464555]/70">
              Todas as sessões ativas foram terminadas. A redirecionar para o login…
            </p>
          </div>
        ) : (
          <>
            <h2 className="text-2xl font-black text-[#101c29] tracking-tight mb-1">Nova password</h2>
            <p className="text-sm text-[#464555]/60 mb-6">Escolhe uma password forte para a tua conta.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 pr-10 rounded-xl text-sm text-[#101c29] outline-none border border-transparent focus:border-[#493ee5]/40 transition-colors"
                    style={{ background: 'rgba(73,62,229,0.05)' }}
                  />
                  <button
                    type="button"
                    onClick={() => setShowPw(v => !v)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#464555]/40 hover:text-[#464555]"
                  >
                ...
                {password.length > 0 && (
                  <ul className="mt-2 space-y-1">
                    {PASSWORD_RULES.map(rule => (
                      <li key={rule.label} className={`flex items-center gap-1.5 text-xs ${rule.test(password) ? 'text-green-600' : 'text-[#464555]/50'}`}>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className={`w-full px-4 py-3 rounded-xl text-sm text-[#101c29] outline-none border transition-colors ${
                    confirm && !passwordsMatch ? 'border-red-400/60' : 'border-transparent focus:border-[#493ee5]/40'
                  }`}
                  style={{ background: 'rgba(73,62,229,0.05)' }}
                />
            ...
            <div className="mt-6 text-center">
              <Link to="/login" className="text-sm text-[#464555]/60 hover:text-[#493ee5] transition-colors">
                Voltar ao login
              </Link>
            </div>
          </>
