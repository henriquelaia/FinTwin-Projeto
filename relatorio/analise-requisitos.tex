...
\textbf{ID} & \textbf{Descrição} & \textbf{Prioridade} & \textbf{Sprint} \\
\midrule
RF36 & O sistema deve classificar automaticamente as transações da plataforma como dedutíveis ou não dedutíveis para efeitos de \ac{IRS} (saúde, educação, habitação, encargos gerais) & Must & 9 \\
RF37 & O sistema deve calcular o \ac{IRS} estimado integrando as deduções identificadas nas transações com os dados da Segurança Social e rendimentos introduzidos pelo utilizador & Must & 9 \\
RF38 & O sistema deve apresentar ao utilizador uma lista detalhada das despesas dedutíveis identificadas nas transações históricas, com valores acumulados por categoria de dedução & Must & 9 \\
RF39 & O sistema deve comparar cenários fiscais alternativos (tributação individual vs.\ conjunta para casais) e recomendar a opção mais vantajosa com base nos rendimentos declarados & Should & 9 \\
RF40 & O sistema deve alertar o utilizador quando as deduções acumuladas numa categoria estiverem próximas do limite legal estabelecido no \ac{CIRS} (e.g.\ saúde: 15\%, limite 1000\,€) & Should & 9 \\
RF41 & O sistema deve estimar o impacto fiscal de despesas futuras planeadas no \ac{IRS} do ano seguinte, com base nos padrões de consumo históricos & Could & 9 \\
\bottomrule
\end{tabularx}
\end{table}
...
A análise de requisitos resultou na identificação de 41 requisitos funcionais e 22 requisitos não-funcionais, organizados em 8 módulos funcionais. A priorização MoSCoW permite focar o desenvolvimento nos requisitos \textit{Must} durante as primeiras sprints, garantindo a entrega de um \ac{MVP} funcional, enquanto os requisitos \textit{Should} e \textit{Could} são implementados progressivamente conforme a disponibilidade de tempo. O Módulo 8, dedicado à \ac{IA} de Otimização Fiscal, representa o contributo técnico mais diferenciador do projeto, introduzindo um sistema de \ac{IA} desenvolvido integralmente sem dependência de serviços externos, fundamentado nas regras oficiais do \ac{CIRS} 2025.
